...
\begin{align*}
&P(0) = \bra{00} M_{0} \ket{00} = \bra{00}\ket{00}\bra{00} \ket{00} = 1 \,.\, 1 = 1\\
...
&P(1) = \bra{01} M_{1} \ket{01} = \bra{01}\ket{01}\bra{01} \ket{01} = 1 \,.\, 1 = 1\\
...
&P(2) = \bra{10} M_{2} \ket{10} = \bra{10}\ket{10}\bra{10} \ket{10} = 1 \,.\, 1 = 1\\
...
&P(3) = \bra{11} M_{3} \ket{11} = \bra{11}\ket{11}\bra{11} \ket{11} = 1 \,.\, 1 = 1\\
\end{align*}
\end{ejemplo}
...
Comenzamos definiendo los primeros componentes que nos permitiran codificar todo el circuito:
...
\begin{align*}
ALICE = \lambda q_a : qbuit. \lambda <b_1,b_2>:bit\otimes bit. \\  
& if \, b_1 \, then \, \\ 
&\qquad if \, b_2 \, then \, Z \, X \, q_a\, \\
&\qquad else \, Z \, q_a\, \\
& else \\
&\qquad if \, b_2 \, then X \, q_a\, \\ 
&\qquad else \, I \, q_a\,:qbuit \rightarrow bit\oplus bit \rightarrow qbuit \\ 
\end{align*}

\begin{align*}
BOB = \lambda q_1. \lambda q_2. \,let <q_1', q_2'> = CNOT <q_1, q_2> in <H \, q_1', q_2'> : qubit\rightarrow qubit \rightarrow qubit \otimes qubit
\end{align*}
...
$$\vdash SDC : !(\mathsf{bit} \multimap \mathsf{bit} \multimap \mathsf{qbit} \otimes \mathsf{qbit} \multimap \mathsf{bit} \otimes \mathsf{bit})$$
...
\[
\vdash \mathtt{SDC} : B \times B \Rightarrow B \times B 
\]
...
\[
{SDC}(\ket{0} \times \ket{0}) = {BOB}\big(({ALICE}\,\ket{0} \times \ket{0})\,{EPR}\big)
\]

Comenzamos con la reduccion de $ALICE$

\begin{align*}
{ALICE}\,\ket{0} \times \ket{0} &\rightarrow
 \lambda q. \\
&\qquad (\lambda q_1.\; (\lambda q_2.\; \\
&\qquad\qquad q_1? \\
&\qquad\qquad\qquad q_2? \\
...
&\qquad\qquad\qquad .\lambda bell. bell \\ 
&\qquad )) {tail}(q))\; {head}(q)  \\ \\
&\text{head}(\ket{0} \times \ket{0}) = \ket{0} \text{ y } {tail}(\ket{0} \times \ket{0})  = \ket{0} \text{ El condicional } q_1?\text{ se evalúa.} \\
...
Una vez definida la reduccion de $ALICE \,\ket{0} \times \,\ket{0}$, tenemos que: 
...
\begin{align*}
(\lambda bell.\,bell)\,\frac{1}{\sqrt{2}}(\ket{00}+\ket{11}  &\rightarrow \frac{1}{\sqrt{2}}(\ket{00}+\ket{11}
\end{align*}

Luego si volvemos a la reduccion de $SDC$ tenemos que:

\begin{align*}
\code{SDC}(\ket{0} \times \ket{0}) = {BOB}\big(({ALICE}\,\ket{0} \times \ket{0})\,{EPR}\big) \rightarrow \code{BOB}(\frac{1}{\sqrt{2}}(\ket{00}+\ket{11})) \\
\end{align*}
\begin{align*}
\code{BOB}(\frac{1}{\sqrt{2}}(\ket{00}+\ket{11}) = \pi_2 \Big( 
                \big(\lambda qn.\; 
                    H(\text{head}\, qn) \times (\text{tail}\, qn)
                \big)(\text{CNOT}\, \frac{1}{\sqrt{2}}(\ket{00}+\ket{11}) 
            \Big) 
\end{align*}
...
\code{BOB}(\frac{1}{\sqrt{2}}(\ket{00}+\ket{11}) = \pi_2 \Big( 
                \big(\lambda qn.\; 
                    H(\text{head}\, qn) \times (\text{tail}\, qn)
                \big)\frac{1}{\sqrt{2}}(\ket{00} + \ket{10}) 
            \Big) 
\end{align*}

El estado donde:
\[
\left(\frac{1}{\sqrt{2}}(\ket{0} + \ket{1})\right) \times \ket{0} = \ket{+} \times \ket{0}
\]

Queda entonces:

\begin{align*}
\code{BOB}(\frac{1}{\sqrt{2}}(\ket{00}+\ket{11})
&= \pi_2 \Big( 
    \big(\lambda qn.\; 
        H(\text{head}\, qn) \times (\text{tail}\, qn)
    \big)(\ket{+} \times \ket{0}) 
...
\[
\code{BOB}(\frac{1}{\sqrt{2}}(\ket{00}+\ket{11}) \rightarrow \pi_2(\ket{00})
\]
\\
Dado que el estado $\ket{00}$ es un término de base con  coeficiente de amplitud  1, la medición resulta en:

\[
\pi_2(\ket{00}) \rightarrow \{1\}\ket{00}
\]

\subsection{Termino en calculo $L_B$}
...
\begin{align*}
&ENCODING = \lambda b_0 : 1+1.\lambda b_1 : 1+1.\lambda q : 1\&1. \delta_{\otimes}(q, yz.(ALICE \, b_0 \,b_1 \,y) \, x \, z) \\ 
&: 1+1 \roundarrow 1+1 \roundarrow 1\&1 \times 1\&1 \roundarrow 1\&1 \times 1\&1
\end{align*}
...
El tipo del termino $SDC$ es:

\begin{align*}
SDC:1+1 \roundarrow 1+1 \roundarrow (1+1 \times 1+1)    
\end{align*}

\subsubsection{Traza de reduccion}

Definimos nuevamente las macros que nos seran de utilidad:
...
\end{align*}
\begin{align*}
EPR = \frac{1}{\sqrt{2}}.(<*,0.*> \times <*,0.*>) + \frac{1}{\sqrt{2}}.(<0.*,*>) \times <0.*,*>)) \\
\end{align*}

Comenzamos la reduccion de $SDC \,0 \,0$:

\begin{align*}
&SDC \, 0 \, 0 = (\lambda b_1 :1+1.\lambda b_2 :1+1. MEAS(BOB(ENCODING \, b_1 \, b_2 \, EPR))) \,0 \,0 \\ \\
&\rightarrow \beta_2 \, MEAS(BOB(ENCODING \, 0 \, 0 \, EPR))) 
\end{align*}

Vemos como reduce $ ENCODING \,0\,0\,EPR$:

\begin{align*}
&ENCODING \, 0 \, 0 \, EPR = (\lambda b_0 : 1+1.\lambda b_1 : 1+1.\lambda q : 1\&1. \delta_{\otimes}(q, yz.(ALICE \, b_0 \,b_1 \,y) \, x \, z)) \,0\,0\,EPR \\
&\xrightarrow{\beta_2} \,\delta_{\otimes}(EPR, yz.(ALICE \, 0 \,0 \,y) \, x \, z) \\
&\xrightarrow{\delta_{\otimes}(12)} \frac{1}{\sqrt{2}} (\delta_{\otimes}(\ket{0} \times \ket{0}, yz.(ALICE \,0 \,0 \,y) \,x \,z)) + \frac{1}{\sqrt{2}} (\delta_{\otimes}(\ket{1} \otimes \ket{1}, yz.(ALICE \,0 \,0 \,y) \,x \,z)) \\
&\xrightarrow{\beta_{\otimes} (7)} \frac{1}{\sqrt{2}} ((ALICE \,0 \,0 \ket{0}) \times \ket{0}) + \frac{1}{\sqrt{2}} ((ALICE \,0 \,0 \ket{1}) \times \ket{1})
\end{align*}

Continuemos con la reduccion de la evacluacion de $ALICE \,0\,0\,q$. Realizamos la reduccion sobre un qubit generico $q$ con el finde de realizar un reduccion mas generalizada:

\begin{align*}
&ALICE \,0 \,0\,q= (\lambda b_1 : 1+ 1.\lambda b_2: 1+1. \lambda q: 1\&1. \delta_{\bigoplus}(b_1, \\ 
&\qquad \qquad \qquad\qquad \qquad x_1.\delta_{\bigoplus}(b_2, x_2.q, y_2.X \,q), \\
&\qquad \qquad \qquad\qquad \qquad\, y_1.\delta_{\bigoplus}(b_2, x_2.Z \, q, y_2. Z \, X \, q)))\,0\,0\,q \\
&\xrightarrow{\beta(2)} \delta_{\oplus}(0, \\
&\qquad \qquad \qquad x1.(\delta_{\oplus}(0,x2.q ,y2.X\,q)), \\
&\qquad \qquad \qquad y1.(\delta_{\oplus}(0,x2.Z\,q, y2. Z\,X\, q))) \\
&\xrightarrow{\beta(5)}\delta_{\oplus}(0, x2.q, y2.X\,q) \\
&\xrightarrow{\beta(5)} q \\
\end{align*}

A partir de la reduccion generica de $ALICE$, si volvemos al $ENCODING$, nos queda:

\begin{align*}
&\frac{1}{\sqrt{2}} ((ALICE \,0 \,0 \ket{0}) x\times\ket{0}) + \frac{1}{\sqrt{2}}  ((ALICE \,0 \,0 \ket{1}) x \ket{1}) \\
&\xrightarrow{Reduccion \,De \,Alice}\frac{1}{\sqrt{2}}(\ket{0} \times \ket{0}) + \frac{1}{\sqrt{2}} (\ket{1} \times \ket{1}) = EPR    
\end{align*}

Si subimos de nuevo a la reduccion de $SDC$: 

\begin{align*}
&MEAS(BOB(ENCODING \,0 \,0 \,EPR)) \\
&\xrightarrow{reduccion\,de\,encoding} MEAS(BOB \,EPR)    
\end{align*}

A partir de esto podemos comenzar con la reduccion de $BOB$

\begin{align*}
&BOB \,EPR = (\lambda q:1\&1. (HxI)(CNOT \,q)) EPR \\
&\xrightarrow{\beta(2)} (HxI)(CNOT \,EPR) \\ 
&= (HxI)(CNOT (\frac{1}{\sqrt{2}} (\ket{0} \times |0>) + \frac{1}{\sqrt{2}} (\ket{1} \times \ket{1})))\\
&= (HxI) (CNOT (\frac{1}{\sqrt{2}}\ket{00} + \frac{1}{\sqrt{2}}\ket{11})) \\
&\xrightarrow{CNOT}(HxI) (\frac{1}{\sqrt{2}}\ket{00} + \frac{1}{\sqrt{2}}\ket{10}) \\
&\xrightarrow{HxI} \frac{1}{\sqrt{2}}(HxI)\ket{00} + \frac{1}{\sqrt{2}}(HxI)\ket{10} \\
&\xrightarrow{HxI}\ket{00} \\
\end{align*}

Luego de haber reducido $BOB$ volvemos a $SDC$ para realizar la medicion final: 

\begin{align*}
&\xrightarrow{reduccion de BOB} MEAS \ket{00} \\
&= (\lambda q. \\
&\qquad \qquad BIT(0,0) . Proj_{00}(q) + \\
&\qquad \qquad BIT(0,1) . Proj_{01}(q) + \\
&\qquad \qquad BIT(1,0) . Proj_{10}(q) + \\
&\qquad \qquad BIT(1,1) . Proj_{11}(q)) |00> \\
&\xrightarrow{\beta(2)} BIT(0,0) . Proj_{00} \ket{00} + BIT(0,1) . Proj_{01} \ket{00} + \\
&\qquad BIT(1,0) . Proj_{10} \ket{00} + BIT(1,1) . Proj_{11} \ket{00}
\end{align*}

Realizamos la reduccion de las proyecciones:

\begin{align*}
&Proj_{00} \ket{00} = \delta_{\otimes} (\ket{00}, xy.(Proj_0 x . Proj_0 y)) \\
&\xrightarrow{\beta_{\otimes}(7)} 
Proj_0 \ket{0} . Proj_0 \ket{0} 
= \delta_{\&^1}(\ket{0},x1.x1) . \delta_{\&^1}(\ket{0},x1.x1) \\
&·\xrightarrow{\beta_{\&^1}(3)}  1 . 1 = 1
\end{align*}

Para $i \neq 0 $ y $j \neq 0$, tenemos que $Proj_{ij} \,\ket{00} \rightarrow*\, 0$

Reducida la medicion volvemos a $SDC$. Teniamos que:

\begin{align*}
&BIT(0,0) . Proj_{00} \ket{00} + BIT(0,1) . Proj_{01} \ket{00} +\\
&BIT(1,0) . Proj_{10} \ket{00} + BIT(1,1) . Proj_{11} \ket{00}
\end{align*}

Luego:

\begin{align*}
&\xrightarrow{Proyeccion}* \,BIT(0,0) \,. \,1 \\ 
&\xrightarrow{Proyeccion}* \,BIT(0,0) \,= inl() \times inl()    
\end{align*}

Donde $inl() \times inl()$ segun lo que definimos no es mas que 0 x 0, o sea el par de bits 0 0.
\end{document}
